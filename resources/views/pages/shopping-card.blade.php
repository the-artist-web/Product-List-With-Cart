...
@section("body")
    <div class="container d-flex align-items-center justify-content-center" style="min-height: calc(100vh - 191px);">
        <div class="bg-second-bg-color rounded-4 p-20 w-100 max-w-550px">
            <h2 class="mb-20 text-capitalize text-orange-color title-medium">your carts ({{ count($carts) }})</h2>
            ...
            @if ($carts->isEmpty())
                <div class="h-350px d-flex flex-column align-items-center justify-content-center gap-4">
                    ...
                    <p class="text-second-color text-capitalize">The shopping cart is empty.</p>
                </div>
            @else
                    ...
                    <div class="max-h-400px overflow-y-scroll scroll-none">
                        @foreach ($carts as $cart)
                            <div class="d-flex align-items-center justify-content-between gap-3">
                                <a href="{{ route("page.product.details", ["id" => $cart->product->id]) }}" class="d-flex align-items-center gap-3">
                                    <img 
                                        src="{{ asset("storage/" . $cart->product->imageProducts->first()->main_image) }}" 
                                    ...
                                    <div class="content">
                                        <p class="truncate-line-2 lh-sm label-medium">{{ $cart->product->title }}</p>
                                        ...
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <p class="text-second-color label-small">${{ $cart->total }}</p>
                                        </div>
                                    </div>
                                </a>
            
                                <form action="{{ route("destroy.cart", ["id" => $cart->id]) }}" method="POST">
                                    @csrf
                                    @method("DELETE")
                ...
                <a href="{{ route("page.checkout") }}" class="btn btn-primary text-uppercase w-100 h-50px label-small">
